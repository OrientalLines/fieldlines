...
EField3d.prototype.StartDraw = function () {
  ...
  for (var i = 0; i < this.charges.length; i++) {
    ...
    urlparams +=
      ...
      "," +
      parseFloat(this.charges[i].z.toFixed(3));
...
EField3d.prototype.SetXYCoords = function (charge, u, v) {
  var XYZVector = Vector.create([
    (u / this.proj_dist) * charge.tranz,
    (v / this.proj_dist) * charge.tranz,
    charge.tranz,
    1,
  ]);
...
EField3d.prototype.FindFieldLines = function () {
  ...
  for (var i = 0; i < this.charges.length; i++) {
    ...
    while (
      ...
      fieldline.start_x =
        charge.x +
        charge.r * Math.sin(start_angle.phi) * Math.cos(start_angle.theta);
      fieldline.start_y =
        charge.y +
        charge.r * Math.sin(start_angle.phi) * Math.sin(start_angle.theta);
...
EField3d.prototype.SeedNodes = function (charge, startangle) {
  ...
  var n = 0;
  var q = 0;
  while (charge.n_nodes > 2 * (n * n - n + 1)) {
    n++;
  }
...
EField3d.prototype.AddArrows = function (fieldline) {
  ...
  if (n > 500) {
    for (var i = 1; i < Math.floor(n / 200); i++) {
      if (n - Math.round(n / Math.floor(n / 200)) * i > 50) {
        this.AddArrow(
          fieldline.points[Math.round(n / Math.floor(n / 200)) * i],
          fieldline.points[
            Math.round(n / Math.floor(n / 200)) * i - fieldline.dir
          ]
        );
      }
    }
    ...
    this.AddArrow(
      fieldline.points[Math.round(n / 2)],
      fieldline.points[Math.round(n / 2) - fieldline.dir]
    );
  }
};
EField3d.prototype.AddArrow = function (point, prevPoint) {
  var arrow = Vector.create([
    point.x - prevPoint.x,
    point.y - prevPoint.y,
    point.z - prevPoint.z,
  ]);
  if (!arrow.isParallelTo(Vector.i)) {
    ...
    var arrowAxis = Line.create(
      [0, 0, 0],
      [arrow.e(1), arrow.e(2), arrow.e(3)]
    );
    for (var i = 0; i < 4; i++) {
      var arrowLine = Vector.create([
        prevPoint.x + p1.e(1) - point.x,
        prevPoint.y + p1.e(2) - point.y,
        prevPoint.z + p1.e(3) - point.z,
      ]);
      this.AddLine(
        point.x,
        point.y,
        point.z,
        point.x + arrowLength * arrowLine.e(1),
        point.y + arrowLength * arrowLine.e(2),
        point.z + arrowLength * arrowLine.e(3),
        1,
        "BLACK",
        null
      );
...
EField3d.prototype.Field = function (x, y, z) {
  ...
  var ret = {
    ...
    gx: Ex / E,
    gy: Ey / E,
    gz: Ez / E, // Field direction vector
  };
...
function RotatePhi(a, angle) {
  ...
  if (angle == 0) return a;
  var k = {
    x: -a.y / Math.sqrt(a.x * a.x + a.y * a.y),
    y: a.x / Math.sqrt(a.x * a.x + a.y * a.y),
    z: 0,
  };
...
function GetDiff(angle1, angle2) {
  var a = {
    x: Math.sin(angle1.phi) * Math.cos(angle1.theta),
    y: Math.sin(angle1.phi) * Math.sin(angle1.theta),
    z: Math.cos(angle1.phi),
  };
  var b = {
    x: Math.sin(angle2.phi) * Math.cos(angle2.theta),
    y: Math.sin(angle2.phi) * Math.sin(angle2.theta),
    z: Math.cos(angle2.phi),
  };
  return Math.abs(Math.acos(Dot(a, b)) % TWO_PI);
}
function GetAngle(a) {
  return {
    theta: Math.atan(a.y / a.x),
    phi: Math.atan(Math.sqrt(a.x * a.x + a.y * a.y) / a.z),
  };
}
function Dot(a, b) {
  ...
  for (var i = 0; i < this.objects.length; i++) {
    ...
    if (obj.source) {
      if (obj.type == "l") {
        ...
        if (
          GeoUtils.line_is_close_to_point(
            u,
            v,
            obj.au,
            obj.av,
...
EField3d.prototype.drag = function (ev) {
  ...
  if (!this.chargeSelected) {
    ...
    if (
      (this.mouse_mode == "pan" && this.drag_button != 3) ||
      (this.mouse_mode == "rotate" && this.drag_button == 3)
    ) {
      // panning
    ...
    for (var i = 0; i < this.charges.length; i++) {
      ...
      var d = Math.sqrt(du * du + dv * dv);
      if (
        d < ((charge.r * this.proj_dist) / charge.tranz) * 1.5 ||
...
EField3d.prototype.startDragging = function (ev) {
  for (var i = 0; i < this.objects.length; i++)
    if (this.objects[i].selected) this.chargeSelected = true;
