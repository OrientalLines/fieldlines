...
function Pad3d(element, options) {
  ...
  if (typeof initCanvas != "undefined")
    this.ctx = initCanvas(this.canvas).getContext("2d");
  ...
  $(this.element).bind("mousedown", function (ev) {
    return self.startDragging(ev);
  });
  $(window).bind("mouseup", function (ev) {
    return self.stopDragging(ev);
  });
  ...
  $(this.element).bind("mousewheel", function (ev, d) {
    return self.scrollMe(ev, -ev.originalEvent.wheelDelta / 100);
  });
  ...
  $(".trid-pan-left", parent)
    ...
    .mousehold(function (ev) {
      self.panMe(ev, -self.pan_increment, 0, 0);
    });
  ...
  $(".trid-pan-up", parent)
    ...
    .mousehold(function (ev) {
      self.panMe(ev, 0, -self.pan_increment, 0);
    });
  ...
  $(".trid-pan-upstream", parent)
    ...
    .mousehold(function (ev) {
      self.panMe(ev, 0, 0, +self.pan_increment);
    });
  ...
  $(".trid-autorotate", parent)
    ...
    .change(function (ev) {
      self.autoRotate($(this).is(":checked"));
    });
  ...
  $(".trid-ctl-mouse-set", parent).buttonset({
  ...
  $(".trid-ctl-view-set", parent).buttonset({
    icons: { primary: "ui-icon-seek-first" },
    text: false,
  });
...
Pad3d.prototype.AddLine = function (
  x1,
  y1,
  z1,
  x2,
...
Pad3d.prototype.AddPoint = function (
  x1,
  y1,
  z1,
  size,
  ...
  if (obj.draw) {
    if (obj.type == "l") {
      ...
      GeoUtils.draw_highlighted_line(
        this.ctx,
        this.width / 2 - obj.au,
        this.height / 2 - obj.av,
        this.width / 2 - obj.bu,
        this.height / 2 - obj.bv,
        this.ctx.lineWidth, //linewidth
        obj.stroke, // default style
        "rgba(250,0,0,0.9)", //highlight style
        "rgba(0,0,0,0.7)", //outline style
...
Pad3d.prototype.scrollMe = function (ev, delta) {
  ...
  if (this.view_mode == "3D") {
    ...
    if (this.camera_distance > this.camera_distance_max)
      this.camera_distance = this.camera_distance_max;
    if (this.camera_distance < this.camera_distance_min)
      this.camera_distance = this.camera_distance_min;
  } else {
    // Orthographic view.
